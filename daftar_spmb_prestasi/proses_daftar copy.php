...
try {
    ...
    $username = $is_new ? '' : mysqli_real_escape_string($kon, $_POST['username']);

    if ($is_new) {
        // Buat username baru
        $username = 'SPMB' . date('YmdHis') . rand(10, 99);

        $insert = "INSERT INTO tb_formulir3 (
            username, nama_lengkap, pilihan_prodi, alamat, jenis_kelamin, agama,
            no_hp, tempat_lahir, tanggal_lahir, berat_badan, tinggi_badan,
            golongan_darah, nik, nama_orang_tua, pekerjaan_orang_tua,
            penghasilan_orang_tua, kartu_bpjs, nama_sekolah, akreditasi,
            asal_sekolah, jurusan_sekolah, daerah_asal, nama_foto,
            email, tanggal_daftar
        ) VALUES (
            '$username', '$nama_lengkap', '$program_studi', '$alamat_detail', '$jenis_kelamin', '$agama',
            '$no_hp', '$tempat_lahir', '$tanggal_lahir', $berat_badan, $tinggi_badan,
            '$golongan_darah', '$nik', '$nama_orang_tua', '$pekerjaan_orang_tua',
            '$penghasilan_orang_tua', '$kartu_bpjs', '$nama_sekolah', '$akreditasi',
            '$asal_sekolah', '$jurusan_sekolah', '$daerah_asal', '$foto_path',
            '$email', NOW()
        )";
