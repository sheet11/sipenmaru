				...
				<table width="100%" class="table table-hover">
					...
					<tr>
						...
						<td><select name='pilihan_prodi' class='form-control'>";
								<option value="<?php echo $a['pilihan_prodi']; ?> ">
									<?php echo $a['pilihan_prodi']; ?></option>
					...
					<tr>
						...
						<td><select name='pilihan_prodi2' class='form-control'>";
								<option value="<?php echo $a['pilihan_prodi2']; ?>">
									<?php echo $a['pilihan_prodi2']; ?></option>
								<?php include "../config/koneksi.php";
								$query = mysqli_query($kon, "SELECT * FROM tb_prodi where aktif=4 order by nama_prodi desc");
								while ($row = mysqli_fetch_array($query)) {
									echo "
					...
					<tr>
						...
						<td><select name='asal_sekolah' class='form-control' required>";
								...
								<?php include "../config/koneksi.php";
								$query = mysqli_query($kon, "SELECT * FROM tb_asal_sekolah WHERE status_sekolah='2' order by asal_sekolah desc");
								while ($row = mysqli_fetch_array($query)) {
									echo "
