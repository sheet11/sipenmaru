        ...
        if ($reservation_token) {
            ...
            try {
                ...
                if ($reservation_data['expires_at'] < $now) {
                    ...
                    if ($upd) {
                        mysqli_stmt_bind_param($upd, "i", $reservation_data['id']);
                ...
                $form_stmt = mysqli_prepare($kon, "SELECT id_formulir, status, username FROM tb_formulir5 WHERE id_formulir = ? FOR UPDATE");
                ...
                $update_sql = "UPDATE tb_formulir5 SET
                    nama_lengkap = ?,
                    pilihan_prodi = ?,
                    alamat = ?,
                    jenis_kelamin = ?,
                ...
                if ($mark_stmt) {
                    mysqli_stmt_bind_param($mark_stmt, "i", $reservation_data['id']);
            ...
            mysqli_begin_transaction($kon, MYSQLI_TRANS_START_READ_WRITE);
            try {
                $stmt = mysqli_prepare($kon, "SELECT id_formulir, status FROM tb_formulir5 WHERE username = ? FOR UPDATE");
                ...
                $types = str_repeat('s', 24) . 's'; // 25 parameter (24 kolom + username)
                mysqli_stmt_bind_param(
                    $stmt2,
                    $types,
                    $nama_lengkap,         // 1
                    ...
                    $nik,                  // 13
                    $nama_orang_tua,       // 14
                    $nama_orang_tua_ibu,   // 15
                    $pekerjaan_orang_tua,  // 16
                    $penghasilan_orang_tua, // 17
                    $pekerjaan_orang_tua_ibu, // 18
                    $kartu_bpjs,           // 19
                    $nama_sekolah,         // 20
                    $akreditasi,           // 21
                    $daerah_asal,          // 22
