            ...
            <table style="width:100%;">
                ...
                <tr>
                    <td><a class="btn btn-info"> Jumlah: <?php
                                                                    ...
                                                                    $query = mysqli_query ($kon,"SELECT * from tb_formulir5 where status='Terdaftar' and tahun_pendaftaran = YEAR(CURDATE())");
...
ini_set('display_errors', 0);
            ...
            ini_set('display_errors', 0);
                if(isset($_POST['submit'])){
                    $cariid = $_POST['cariid'];
                    $cari = $_POST['cari'];
                    $query=mysqli_query($kon,"select * from tb_formulir5 where status='Terdaftar' and tahun_pendaftaran = YEAR(CURDATE()) and $cariid = '$cari' or $cariid = '0' "); 
                    $i = $posisi+1;      
            ...
            echo"
                ...
                elseif(!empty($_GET['username'])){
                    $query=mysqli_query($kon,"select * from tb_formulir5 where status='Terdaftar' and tahun_pendaftaran = YEAR(CURDATE()) and username='$_GET[username]'"); 
                    $i = $posisi+1;      
                while($a=mysqli_fetch_array($query)){
            echo"
                <tr>    
                ...
                $i++;
                    ...
                    $query=mysqli_query($kon,"select * from tb_formulir5 where status='Terdaftar' and tahun_pendaftaran = YEAR(CURDATE()) order by username asc  LIMIT $posisi,$batas");
                    error_reporting(E_ALL);
ini_set('display_errors', 0);
    ...
    $jmldata = mysqli_num_rows(mysqli_query($kon,"SELECT * FROM tb_formulir5 where status='Terdaftar' and tahun_pendaftaran = YEAR(CURDATE())"));
