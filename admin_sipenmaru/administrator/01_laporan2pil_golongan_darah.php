                ...
                foreach ($prodi_list as $label => $prodi_value): ?>
                    <tr>
                        ...
                        <?php foreach ($golongan_darah_list as $golongan_darah_label => $golongan_darah_values):
                            ...
                            if ($prodi_value === "") {
                                $query = mysqli_query($kon, "SELECT 1 FROM tb_formulir4 WHERE (status='Sudah Membayar' OR status='Terdaftar') AND golongan_darah IN ($in_values) AND tahun_pendaftaran='2026'");
                            } else {
                                $query = mysqli_query($kon, "SELECT 1 FROM tb_formulir4 WHERE pilihan_prodi = '$prodi_value' AND (status='Sudah Membayar' OR status='Terdaftar') AND golongan_darah IN ($in_values) AND tahun_pendaftaran='2026'");
                            }
