...
<aside class="right-side">
        ...
        <div class="container-fluid" style="margin:10px;">
            <table style="width:100%;">
                <tr class="info">
                    <td align="left" colspan="6"><b>
                            <h4>Daftar Calon Mahasiswa Baru 1 Pilihan Mandiri
                        </b></h4>
                    </td>
                ...
                <tr>
                    <td><a class="btn btn-info"> Jumlah: <?php
                                                                    ...
                                                                    $query = mysqli_query ($kon,"SELECT * from tb_formulir5 where status='Belum Lengkap' and tahun_pendaftaran = YEAR(CURDATE())");
                    ...
                    ?>
                        <a class="btn btn-success"
                            href="082_cetak_excel_calon_mahasiswa.php?cariid=<?php echo $cariid; ?>&cari=<?php echo $cari; ?>">Cetak
                            Excel</a>
                        <?php
                    }
                    ...
                    ?>
                        <a class="btn btn-success"
                            href="082_cetak_excel_calon_mahasiswa.php?username=<?php echo $_GET['username']; ?>">Cetak
                            Excel</a>
                        <?php
                    }
                    else
            ...
            include "../config/koneksi.php";
                if(isset($_POST['submit'])){
                    $cariid = $_POST['cariid'];
                    $cari = $_POST['cari'];
                    $query=mysqli_query($kon,"select * from tb_formulir5 where status='Belum Lengkap' and tahun_pendaftaran = YEAR(CURDATE()) and $cariid = '$cari' or $cariid = '0' "); 
                    $i = $posisi+1;      
            ...
            echo"
                ...
                elseif(!empty($_GET['username'])){
                    $query=mysqli_query($kon,"select * from tb_formulir5 where status='Belum Lengkap' and username='$_GET[username]' and tahun_pendaftaran = YEAR(CURDATE())"); 
                    $i = $posisi+1;      
                while($a=mysqli_fetch_array($query)){
            echo"
                <tr>    
                ...
                $i++;
                    ...
                    $query=mysqli_query($kon,"select * from tb_formulir5 where status='Belum Lengkap' and tahun_pendaftaran = YEAR(CURDATE()) order by username asc  LIMIT $posisi,$batas");
    ...
    $jmldata = mysqli_num_rows(mysqli_query($kon,"SELECT * FROM tb_formulir5 where status='Belum Lengkap' and tahun_pendaftaran = YEAR(CURDATE())"));
