<?php
	...
	if (isset($_GET['id_periode'])) {
		...
		if ($stmt->execute()) {
			...
			echo "<script>alert('Gagal menghapus data');window.location='periode.php'</script>";
		}

		// Close the statement
		$stmt->close();
	} else {
		echo "<script>alert('ID periode tidak ditemukan');window.location='periode.php'</script>";
	}
