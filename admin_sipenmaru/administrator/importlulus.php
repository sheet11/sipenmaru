 ...
 if(in_array($extension, $allowed_extension)) //check selected file extension is present in allowed extension array
 {
  ...
  foreach ($objPHPExcel->getWorksheetIterator() as $worksheet)
  {
   ...
   for($row=2; $row<=$highestRow; $row++)
   {
    ...
    $status_pmdp = trim(mysqli_real_escape_string($kon, $worksheet->getCellByColumnAndRow(2, $row)->getValue())," ");

    if(!empty($username)) // if none of the data are empty
    {
      $getmhs=mysqli_fetch_array(mysqli_query($kon,"select * from tb_formulir3 where username='$username'"));
      if(!empty($getmhs['username']))
      {
        mysqli_query($kon,"update tb_formulir3 set status_pmdp='$status_pmdp' where username='$username'");  
        ?>
        <div class="alert alert-success">Status PMDP berhasil diubah untuk username : <?php echo $username.' Nama : '.$nama.' dengan status <b>'.$status_pmdp.'</b>';?>
        </div>
        <?php
      }
      else
      {
        ?>
        <div class="alert alert-danger">Status PMDP gagal diubah untuk username : <?php echo $username.' Nama : '.$nama.' dengan status <b>'.$status_pmdp.'</b>. Data tidak ditemukan';?>
        </div>
        <?php
      }
