    ...
    if ($is_new) {
        ...
        if ($reservation_token) {
            ...
            try {
                ...
                $form_stmt = mysqli_prepare($kon, "SELECT id_formulir, status, username FROM tb_formulir4 WHERE id_formulir = ? FOR UPDATE");
                ...
                $update_sql = "UPDATE tb_formulir4 SET
                    nama_lengkap = ?,
                    pilihan_prodi = ?,
                    pilihan_prodi2 = ?,
                    alamat = ?,
            ...
            mysqli_begin_transaction($kon, MYSQLI_TRANS_START_READ_WRITE);
            try {
                $stmt = mysqli_prepare($kon, "SELECT id_formulir, status FROM tb_formulir4 WHERE username = ? FOR UPDATE");
                ...
                $types = 'ssssssssssssssssssssssssssssss'; // 29 parameter (semua string)
                mysqli_stmt_bind_param(
                    $stmt2,
                    $types,
                    $nama_lengkap,         // 1
                    ...
                    $nik,                  // 15
                    $nama_orang_tua,       // 16
                    $nama_orang_tua_ibu,   // 17
                    $pekerjaan_orang_tua,  // 18
                    $penghasilan_orang_tua,// 19
                    $pekerjaan_orang_tua_ibu, // 20
                    $kartu_bpjs,           // 21
                    $nama_sekolah,         // 22
                    $akreditasi,           // 23
                    $asal_sekolah,         // 24
