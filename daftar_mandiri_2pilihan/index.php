    ...
    <?php
	include "koneksi.php";
	$ambil = mysqli_fetch_array(mysqli_query($kon, "SELECT username, password, id_formulir FROM tb_formulir4 WHERE status='Belum Lengkap' ORDER BY id_formulir ASC LIMIT 1"));
	?>
    <div id="wrapper">
    	<nav class="navbar navbar-default navbar-cls-top " role="navigation" style="margin-bottom: 0">
    		<div class="navbar-header">
    			...
    			<a class="navbar-brand" href="index.php">Sistem Penerimaan Mahasiswa Baru Jalur Mandiri 2 Pilihan<br><i style="color: red;">Minimal lulusan SMA/MA/SMK/Sederajat</i></a>
    		</div>
		...
		if (isset($_POST['submit'])) {
			...
			$ambil = mysqli_fetch_array(mysqli_query($kon, "SELECT * FROM tb_formulir4 WHERE username='$username' AND status='Terdaftar'"));
			if ($ambil > 0) {
				...
				$dapat = mysqli_query($kon, "UPDATE tb_formulir4 SET nama_lengkap = '$_POST[nama_lengkap]',
														no_hp = '$_POST[no_hp]',
