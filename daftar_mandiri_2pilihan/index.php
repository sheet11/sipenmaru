...
<head>
	...
	<title>SPMB Jalur Mandiri 2 Pilihan</title>
	<!-- CSS -->
...
if (!$ambil) {
	$ambil = mysqli_fetch_array(mysqli_query($kon, "SELECT id_formulir, username, password FROM tb_formulir4 WHERE status='Belum Lengkap' ORDER BY id_formulir ASC LIMIT 1"));
}
?>
<?php
$date = date("Y-m-d");
...
<body style="text-align: center;">
	...
	<script>
		...
		const programStudi1Select = document.getElementById('program_studi_1');
		const programStudi2Select = document.getElementById('program_studi_2');
		...
		// Helper untuk update program studi options untuk dua dropdown
		function updateProgramStudiDouble(programList) {
			// Untuk Program Studi 1
			programStudi1Select.innerHTML = '';
			let defaultOpt1 = document.createElement('option');
			defaultOpt1.value = '';
			defaultOpt1.textContent = '-- Pilih Program Studi 1 --';
			programStudi1Select.appendChild(defaultOpt1);
			// Untuk Program Studi 2
			programStudi2Select.innerHTML = '';
			let defaultOpt2 = document.createElement('option');
			defaultOpt2.value = '';
			defaultOpt2.textContent = '-- Pilih Program Studi 2 --';
			programStudi2Select.appendChild(defaultOpt2);

			if (programList) {
				programList.forEach(prog => {
					let opt1 = document.createElement('option');
					opt1.value = prog.value;
					opt1.textContent = prog.text;
					programStudi1Select.appendChild(opt1);

					let opt2 = document.createElement('option');
					opt2.value = prog.value;
					opt2.textContent = prog.text;
					programStudi2Select.appendChild(opt2);
				});
			}
			// Reset selection
			programStudi1Select.value = '';
			programStudi2Select.value = '';
		}
		...
		function syncProdi2Options() {
			const selected1 = programStudi1Select.value;
			for (let i = 0; i < programStudi2Select.options.length; i++) {
				let opt = programStudi2Select.options[i];
				if (opt.value !== '') {
					opt.disabled = (opt.value === selected1);
				}
			}
			// Jika prodi 2 yang dipilih sama dengan prodi 1, reset
			if (programStudi2Select.value === selected1) {
				programStudi2Select.value = '';
			}
		}

		function syncProdi1Options() {
			const selected2 = programStudi2Select.value;
			for (let i = 0; i < programStudi1Select.options.length; i++) {
				let opt = programStudi1Select.options[i];
				if (opt.value !== '') {
					opt.disabled = (opt.value === selected2);
				}
			}
			// Jika prodi 1 yang dipilih sama dengan prodi 2, reset
			if (programStudi1Select.value === selected2) {
				programStudi1Select.value = '';
			}
		}
		programStudi1Select.addEventListener('change', syncProdi2Options);
		programStudi2Select.addEventListener('change', syncProdi1Options);
		//batas pilih prodi
		// ========================================
